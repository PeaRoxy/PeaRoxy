...
namespace PeaRoxy.Windows
{
    ...
    public static class Common
    {
        ...
        /// </param>
        /// <param name="autoExit">
        ///     Automatically close the process in case of parent crash or termination
        /// </param>
        /// <param name="usePath">
        ///     Not appending current directory address to file name
        /// </param>
        /// <returns>
            ...
            bool usePath = false)
        {
                                          ...
                                          args)
                                              {
                                                  ...
                                                  WorkingDirectory = Path.GetDirectoryName(Path.GetFullPath(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) ?? string.Empty, address))) ?? string.Empty,
                                              }
                                  };
            ...
            if (autoExit)
            {
                Job processJob = new Job();
                processJob.SetBasicLimitInformation(
                    new JobObjectBasicLimitInformation { Limits = LimitFlags.JobObjectLimitKillOnJobClose });
